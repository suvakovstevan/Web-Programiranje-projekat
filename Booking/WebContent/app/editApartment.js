...
<div class="col-md-6 divider"></div>  
          ...
          <div class="row">
              ...
              <form>
				...
				<div class="col-md-3 divider">
                  <input type="input" class="btn btn-search btn-lg" value="Save" v-bind:disabled="mode=='BROWSE'" v-on:click="saveApartment(apartment)"/>
				</div>
				<div class="col-md-3 divider">
                  <input type="input" class="btn btn-search btn-lg" value="Cancel" v-bind:disabled="mode=='BROWSE'" v-on:click="cancelEditing()"/>
				</div>
                </div>
              </form>
	...
	mounted(){
		...
		this.backup=JSON.parse(localStorage.currentApartment);
		console.log(this.backup)
		axios.get('http://localhost:8080/Booking/rest/amenities/all').then((response) =>{this.amenities=response.data},(error) => {console.log(error.response.data)})	
	},
	components : {
		vuejsDatepicker
	},
	methods : {
		cancelEditing : function(){
			this.apartment=this.backup;
			this.mode='BROWSE'
			this.$router.go('/allApartments')
		},
		saveApartment : function(apartment){
			axios.put('http://localhost:8080/Booking/rest/apartments/modify', apartment).then((response)=>{console.log(response.data)},(error)=>{console.log(error.response.data)})
			this.mode='BROWSE';
			this.$router.go('/')
		}
	}
